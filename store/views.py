...
# view for the categories being fectehd from the db
def category_view(request, category_name):
    category = get_object_or_404(Category, name=category_name)
    products = Product.objects.filter(category=category)
    return render(
        request, "add_category.html", {"category": category, "products": products}
    )
...
def add_category(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "category_summary"
            )  # Redirect to a list of categories or wherever appropriate
    else:
        form = CategoryForm()
